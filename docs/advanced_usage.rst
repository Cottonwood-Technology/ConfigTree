...
*   Add a value to the current value. If current value and the added value are number, a sum is usded. 
    If it is a collection, the value is append to it otherwise the value and the current value are
    converted to string and concat together with a space character as separator.
